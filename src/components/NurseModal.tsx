...
export const NurseModal: React.FC<NurseModalProps> = ({
  ...
  const { specialDutyOptions, isAdmin } = useHemo();
  ...
  useEffect(() => {
    if (nurse) {
      ...
      const knownCodes = new Set(specialDutyOptions.map((o) => o.code));
      const standardDuties = parsed.filter((d) => knownCodes.has(d));
      const customDuties = parsed.filter((d) => !knownCodes.has(d));
          ...
          <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-4 text-xs sm:text-sm">
            ...
            <div className="p-4 bg-slate-50/80 dark:bg-slate-800/40 rounded-2xl border border-slate-200 dark:border-slate-700/80 space-y-3">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {isAdmin && (
                        <button
                          ...
                          className="inline-flex items-center gap-1 text-[10px] font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-200 bg-indigo-50 dark:bg-indigo-950/80 px-2 py-0.5 rounded-lg border border-indigo-200 dark:border-indigo-800 cursor-pointer"
                          title="Tambah, ubah, atau hapus daftar pilihan tugas khusus"
                        >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-1">
                {specialDutyOptions.map((info) => {
                  const isSelected = selectedDuties.includes(info.code);
                  return (
                    <button
                      key={info.id || info.code}
                      type="button"
                      onClick={() => handleToggleDuty(info.code)}
                      className={`p-2 rounded-xl text-left border transition-all text-xs flex flex-col justify-between min-h-[58px] cursor-pointer ${
                        isSelected
              ...
              {(selectedDuties.length > 0 || (isCustomDuty && customDutyText.trim())) && (
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {selectedDuties.map((dCode) => {
                    const opt = specialDutyOptions.find((o) => o.code === dCode) || SPECIAL_DUTY_OPTIONS[dCode];
                    return (
                      <span
                        key={dCode}
              ...
              {selectedDuties.length > 0 && (
                <div className="space-y-1.5 pt-1">
                  {selectedDuties.map((dutyCode) => {
                    const opt = specialDutyOptions.find((o) => o.code === dutyCode) || SPECIAL_DUTY_OPTIONS[dutyCode];
                    if (!opt) return null;
                    return (
                      <div
                        key={dutyCode}
