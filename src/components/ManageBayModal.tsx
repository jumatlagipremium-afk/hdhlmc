...
export const ManageBayModal: React.FC<ManageBayModalProps> = ({
  ...
  const [applyStatusToAll, setApplyStatusToAll] = useState<boolean>(true);
  const [targetShift, setTargetShift] = useState<'ALL' | 'PAGI' | 'SIANG'>('ALL');
  ...
  // Quick 1-click batch status for all machines in this bay
  const handleQuickSetAllStatus = (status: MachineStatus) => {
    if (!isAdmin) {
      showToast('Hanya Kepala Ruangan (Karu) atau Admin yang berwenang mengubah status Bay.', 'info');
      return;
    }
    setBayMachinesStatus(selectedBay, status, targetShift);
    setSelectedStatus(status);
    setApplyStatusToAll(true);
  };

  // Change individual machine status inside the bay
  const handleMachineStatusChange = (machine: Machine, newStatus: MachineStatus) => {
    if (!isAdmin) {
      showToast('Hanya Kepala Ruangan (Karu) atau Admin yang berwenang mengubah status mesin.', 'info');
      return;
    }
    const updated: Machine = {
      ...machine,
      status: targetShift === 'ALL' ? newStatus : machine.status,
      statusPagi: targetShift === 'ALL' || targetShift === 'PAGI' ? newStatus : machine.statusPagi,
      statusSiang: targetShift === 'ALL' || targetShift === 'SIANG' ? newStatus : machine.statusSiang,
    };
    updateMachine(updated);
    showToast(`Mesin ${machine.code} diatur ke ${newStatus}`, 'success');
  };
          ...
          {activeTab === 'EDIT_BAY' && (
            <form onSubmit={handleSaveBayChanges} className="space-y-4">
              ...
              <div className="bg-white p-4 rounded-2xl border-2 border-slate-300 shadow-xs space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div>
                    ...
                    <span className="text-[11px] text-slate-500 font-medium">
                      Atur status seluruh atau sebagian mesin yang berada di {selectedBay}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl border border-slate-200 self-start sm:self-auto">
                    {(
                      [
                        { id: 'ALL', label: 'Semua Sif' },
                        { id: 'PAGI', label: 'Sif Pagi' },
                        { id: 'SIANG', label: 'Sif Siang' },
                      ] as const
                    ).map((sh) => (
                      <button
                        key={sh.id}
                        type="button"
                        onClick={() => setTargetShift(sh.id)}
                ...
                {/* Live Status Breakdown for this Bay */}
                {currentBayMachines.length > 0 && (
                  <div className="flex flex-wrap items-center gap-2 p-2 bg-slate-50 rounded-xl border border-slate-200 text-xs font-bold text-slate-700">
                    ...
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[11px]">
                      ...
                      {
                        currentBayMachines.filter((m) => {
                          const st = targetShift === 'ALL' ? m.status : targetShift === 'PAGI' ? (m.statusPagi || m.status) : (m.statusSiang || m.status);
                          return (st || 'AKTIF') === 'AKTIF';
                        }).length
                      } Aktif
                    </span>
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 text-[11px]">
                    ...
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-200 text-slate-800 text-[11px]">
                      <span className="w-1.5 h-1.5 rounded-full bg-slate-500" />
                      {
                        currentBayMachines.filter((m) => {
                          const st = targetShift === 'ALL' ? m.status : targetShift === 'PAGI' ? (m.statusPagi || m.status) : (m.statusSiang || m.status);
                          return st === 'TIDAK_DIGUNAKAN';
                        }).length
                      } Off
                    </span>
                    {currentBayMachines.some((m) => {
                      const st = targetShift === 'ALL' ? m.status : targetShift === 'PAGI' ? (m.statusPagi || m.status) : (m.statusSiang || m.status);
                      return st === 'RUSAK';
                    }) && (
                ...
                {/* Checkbox: Apply to all machines */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-1 border-t border-slate-200">
                  <label className="flex items-center gap-2 text-xs font-bold text-slate-800 cursor-pointer">
                    ...
                    />
                    <span>Terapkan status &quot;{MACHINE_STATUS_INFO[selectedStatus].label}&quot; ke seluruh mesin di Bay ini</span>
                  </label>
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <button
                      type="button"
                      onClick={() => handleQuickSetAllStatus('AKTIF')}
                      className="px-2.5 py-1 rounded-lg bg-emerald-50 hover:bg-emerald-100 border border-emerald-300 text-emerald-800 text-[11px] font-extrabold cursor-pointer transition-colors active:scale-95"
                      title="Set semua mesin di Bay ini menjadi Aktif"
                    >
                      ✓ Set Semua Aktif
                    </button>
                    <button
                      type="button"
                      onClick={() => handleQuickSetAllStatus('MAINTENANCE')}
                      className="px-2.5 py-1 rounded-lg bg-amber-50 hover:bg-amber-100 border border-amber-300 text-amber-800 text-[11px] font-extrabold cursor-pointer transition-colors active:scale-95"
                      title="Set semua mesin di Bay ini menjadi Maintenance"
                    >
                      ⚠ Set Maintenance
                    </button>
                    <button
                      type="button"
                      onClick={() => handleQuickSetAllStatus('TIDAK_DIGUNAKAN')}
                      className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 border border-slate-300 text-slate-700 text-[11px] font-extrabold cursor-pointer transition-colors active:scale-95"
                      title="Set semua mesin di Bay ini menjadi Tidak Digunakan"
                    >
                      Off Semua
                    </button>
                  </div>
              ...
              {/* Machines Currently in this Bay with Individual Status Adjustments */}
              <div className="bg-white p-4 rounded-2xl border-2 border-slate-300 shadow-xs space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[11px] text-slate-500 font-medium">
                    Ubah status per mesin atau pindah Bay
                  </span>
                </div>
                ...
                {currentBayMachines.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-56 overflow-y-auto p-1">
                    {currentBayMachines.map((m) => {
                      const effStatus = targetShift === 'ALL'
                        ? (m.status || 'AKTIF')
                        : targetShift === 'PAGI'
                        ? (m.statusPagi || m.status || 'AKTIF')
                        : (m.statusSiang || m.status || 'AKTIF');

                      return (
                        <div
                          key={m.id}
                          className="p-2.5 rounded-xl border border-slate-200 bg-slate-50 flex flex-col gap-2 text-xs hover:border-slate-300 transition-colors"
                        >
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex items-center gap-1.5 min-w-0">
                              <span className="font-black text-slate-900 bg-white px-2 py-0.5 rounded border border-slate-200 text-xs">
                                {m.code}
                              </span>
                              <span className="font-bold text-slate-800 truncate text-xs">{m.name}</span>
                            </div>
                            <span
                            ...
                            >
                              {MACHINE_STATUS_INFO[effStatus]?.label || effStatus}
                            </span>
                          </div>

                          <div className="flex items-center gap-1.5">
                            {/* Quick status dropdown for this individual machine */}
                            <select
                              value={effStatus}
                              onChange={(e) => handleMachineStatusChange(m, e.target.value as MachineStatus)}
                              className="text-[11px] font-extrabold bg-white border border-slate-300 rounded-lg px-2 py-1 text-slate-800 cursor-pointer flex-1 focus:ring-1 focus:ring-blue-500"
                              title="Ubah status mesin ini"
                            >
                              <option value="AKTIF">🟢 Aktif</option>
                              <option value="MAINTENANCE">🟡 Maintenance</option>
                              <option value="TIDAK_DIGUNAKAN">⚪ Off (Tidak Digunakan)</option>
                            ...
                            <select
                              ...
                              className="text-[11px] font-bold bg-white border border-slate-300 rounded-lg px-2 py-1 text-slate-700 cursor-pointer flex-1 focus:ring-1 focus:ring-blue-500"
