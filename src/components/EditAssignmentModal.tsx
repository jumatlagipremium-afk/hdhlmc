...
export const EditAssignmentModal: React.FC<EditAssignmentModalProps> = ({
  ...
  const { specialDutyOptions } = useHemo();
  ...
  useEffect(() => {
    if (assignment) {
      ...
      const standard = parsed.filter((d) => knownCodes.has(d));
      const custom = parsed.filter((d) => !knownCodes.has(d));
  ...
  const handleToggleLeader = (newLeader: boolean) => {
    setIsLeader(newLeader);
    setSelectedDuties((prev) => {
      if (newLeader) {
        if (!prev.some((d) => d.toUpperCase().includes('PJ') || d.toUpperCase().includes('LEADER'))) {
          return [...prev, 'PJ SHIF'];
        }
        return prev;
      } else {
        return prev.filter((d) => !d.toUpperCase().includes('PJ') && !d.toUpperCase().includes('LEADER'));
      }
    });
  };

  const handleToggleDuty = (code: string) => {
    const isPjCode = code.toUpperCase().includes('PJ') || code.toUpperCase().includes('LEADER');
    setSelectedDuties((prev) => {
              ...
              {isWorkShift && (
                <div className="p-3 bg-amber-50/80 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 rounded-2xl flex items-center justify-between">
                  ...
                  <input
                    type="checkbox"
                    checked={isLeader}
                    onChange={(e) => handleToggleLeader(e.target.checked)}
                ...
                <div className="p-3.5 bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-2.5">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                    {specialDutyOptions.map((opt) => {
                      const isSelected = selectedDuties.includes(opt.code);
                      const dot = getSpecialDutyDotStyle(opt.code, specialDutyOptions);
                      return (
                        <button
                          key={opt.id || opt.code}
                          type="button"
                          onClick={() => handleToggleDuty(opt.code)}
                          className={`p-2 rounded-xl border text-left text-[11px] transition-all flex items-center justify-between gap-1.5 cursor-pointer ${
                            isSelected
