...
import { Nurse, NURSE_ROLE_INFO, parseSpecialDuties } from '../types';
import { NurseModal } from '../components/NurseModal';
import { DeleteConfirmModal } from '../components/DeleteConfirmModal';
import { SpecialDutyBadge } from '../components/SpecialDutyBadge';
import { ManageSpecialDutiesModal } from '../components/ManageSpecialDutiesModal';
    ...
    <div className="pb-24 space-y-4">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-4 sm:p-6 shadow-xs border border-slate-200/90 dark:border-slate-800 space-y-4">
          ...
          {isAdmin && (
            <div className="flex items-center gap-2 self-stretch sm:self-auto flex-wrap">
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 dark:bg-indigo-950/70 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 rounded-2xl text-xs font-bold transition-all min-h-[44px] cursor-pointer"
                title="Tambah, ubah, atau hapus pilihan nama tugas khusus"
              >
                <SlidersHorizontal className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" />
                <span>Kelola Tugas Khusus</span>
              </button>
              <button
                onClick={() => {
                  setEditingNurse(null);
        ...
        <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2 overflow-x-auto text-[11px] pb-1">
          ...
          {specialDutyOptions.map((opt) => {
            const count = stats.dutyCounts[opt.code] || 0;
            const assignedNurse = nurses.find((n) => n.specialDuty && n.specialDuty.includes(opt.code));
            const isActiveFilter = dutyFilter === opt.code;
            return (
              <button
                key={opt.code}
                onClick={() => setDutyFilter(isActiveFilter ? 'ALL' : opt.code)}
                className={`px-2.5 py-1 rounded-xl font-bold transition-all shrink-0 flex items-center gap-1.5 border cursor-pointer ${
                  isActiveFilter
          ...
          {isAdmin && (
            <button
              onClick={() => setIsManageDutiesOpen(true)}
              className="ml-auto inline-flex items-center gap-1 px-2 py-1 rounded-xl font-bold transition-all shrink-0 text-[10px] text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/80 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 border border-indigo-200 dark:border-indigo-800 cursor-pointer"
              title="Kelola daftar pilihan tugas khusus"
            >
      ...
      {/* Kelola Pilihan Tugas Khusus Modal */}
