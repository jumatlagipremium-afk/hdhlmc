...
export interface HemoContextType {
  ...
  resetMonthlySchedule: (monthStr?: string) => Promise<boolean>;
  ...
  triggerAutoSaveToGoogleSheets: (immediate?: boolean) => void;
}
...
export const HemoProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [settings, setSettings] = useState<AppSettings>(() => {
    ...
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        if (!parsed.googleSheetWebhookUrl && INITIAL_SETTINGS.googleSheetWebhookUrl) {
          parsed.googleSheetWebhookUrl = INITIAL_SETTINGS.googleSheetWebhookUrl;
        }
        if (!parsed.googleSpreadsheetIdOrUrl && INITIAL_SETTINGS.googleSpreadsheetIdOrUrl) {
  ...
  const PURGED_SPECIAL_DUTY_CODES = new Set([
    'REUSE DIALYZER',
    'IPCN / PPI HD',
    'KLAIM & DOKUMEN BPJS',
    'REUSE',
      ...
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          const filtered = parsed
            .filter(
              (o: SpecialDutyOption) =>
                !PURGED_SPECIAL_DUTY_CODES.has(o.code?.toUpperCase()) &&
                o.id !== 'REUSE_DIALYZER' &&
                o.id !== 'IPCN_PPI' &&
                o.id !== 'BPJS'
            )
            .map((o: SpecialDutyOption) => {
              if (o.code === 'FARMASI & LOGISTIK') {
                return { ...o, code: 'FARMASI LOGISTIK', shortName: 'FARMASI LOGISTIK' };
              }
              return o;
            });
  ...
  // Google Sheets Sync & Auto-Save State
  const [sheetsSyncStatus, setSheetsSyncStatus] = useState<SheetsSyncStatus>('idle');
  const [sheetsSyncMessage, setSheetsSyncMessage] = useState<string>('Tersinkron dengan Google Sheets');
  const [lastSheetsSyncTime, setLastSheetsSyncTime] = useState<number | null>(() => {
    const saved = localStorage.getItem('hemo_last_sheets_sync_time');
    return saved ? Number(saved) : null;
  });

  const autoSaveTimerRef = useRef<NodeJS.Timeout | null>(null);
  const hasAutoFetchedOnMount = useRef<boolean>(false);

  // References to ensure debounced closures always use latest state
  const nursesRef = useRef(nurses);
  const machinesRef = useRef(machines);
  const assignmentsRef = useRef(assignments);
  const baysRef = useRef(bays);
  const doctorsRef = useRef(doctors);
  const doctorDutiesRef = useRef(doctorDuties);
  const specialDutyOptionsRef = useRef(specialDutyOptions);
  const settingsRef = useRef(settings);

  useEffect(() => { nursesRef.current = nurses; }, [nurses]);
  useEffect(() => { machinesRef.current = machines; }, [machines]);
  useEffect(() => { assignmentsRef.current = assignments; }, [assignments]);
  useEffect(() => { baysRef.current = bays; }, [bays]);
  useEffect(() => { doctorsRef.current = doctors; }, [doctors]);
  useEffect(() => { doctorDutiesRef.current = doctorDuties; }, [doctorDuties]);
  useEffect(() => { specialDutyOptionsRef.current = specialDutyOptions; }, [specialDutyOptions]);
  useEffect(() => { settingsRef.current = settings; }, [settings]);

  // Auto-Save di Latar Belakang (dengan Debounce & Indikator Status)
  const triggerAutoSaveToGoogleSheets = (immediate: boolean = false) => {
    const webhookUrl = settingsRef.current.googleSheetWebhookUrl?.trim();
    if (!webhookUrl || !webhookUrl.includes('/exec')) {
      return;
    }

    if (typeof navigator !== 'undefined' && !navigator.onLine) {
      setSheetsSyncStatus('offline');
      setSheetsSyncMessage('Koneksi terputus. Data jadwal tersimpan di perangkat.');
      return;
    }

    // Ubah status ke "Menyimpan..." secara instan pada indikator
    setSheetsSyncStatus('saving');
    setSheetsSyncMessage('Menyimpan...');

    if (autoSaveTimerRef.current) {
      clearTimeout(autoSaveTimerRef.current);
    }

    const delayMs = immediate ? 350 : 2000;
    ...
    autoSaveTimerRef.current = setTimeout(async () => {
      try {
        ...
        const res = await GoogleSheetsService.syncAllToGoogleSheets(
          ...
          baysRef.current,
          doctorsRef.current,
          doctorDutiesRef.current,
          specialDutyOptionsRef.current
        );

        if (res.isSuccess) {
          setSheetsSyncStatus('saved');
          setSheetsSyncMessage('Semua data tersimpan otomatis di Google Sheets');
          const now = Date.now();
          setLastSheetsSyncTime(now);
          localStorage.setItem('hemo_last_sheets_sync_time', String(now));
          setSettings((prev) => ({
            ...prev,
            lastSyncTimestamp: now,
            lastSyncStatus: 'Semua data tersimpan otomatis di Google Sheets',
          }));
        } else {
          setSheetsSyncStatus('error');
          setSheetsSyncMessage(res.message || 'Gagal menyimpan ke Google Sheets');
        }
      } catch {
        setSheetsSyncStatus('error');
        setSheetsSyncMessage('Koneksi terputus. Data tersimpan di HP/PC.');
      }
    }, delayMs);
  };

  // Auto-Fetch saat Aplikasi Dibuka (Menarik data terbaru dari Google Sheets tanpa perlu klik manual)
  useEffect(() => {
    if (hasAutoFetchedOnMount.current) return;
    const webhookUrl = settings.googleSheetWebhookUrl?.trim();
    if (!webhookUrl || !webhookUrl.includes('/exec')) return;

    hasAutoFetchedOnMount.current = true;

    const performAutoFetch = async () => {
      if (typeof navigator !== 'undefined' && !navigator.onLine) {
        setSheetsSyncStatus('offline');
        setSheetsSyncMessage('Mode Offline. Menggunakan data tersimpan di perangkat.');
        return;
      }

      setSheetsSyncStatus('fetching');
      setSheetsSyncMessage('Memperbarui data dari Google Sheets...');

      try {
        const monthStr = `${selectedYear}-${String(selectedMonth).padStart(2, '0')}`;
        const res = await GoogleSheetsService.fetchDataFromGoogleSheets(webhookUrl, monthStr);
        if (res.isSuccess) {
          if (res.nurses && res.nurses.length > 0) {
            setNurses(res.nurses);
            localStorage.setItem('hemo_nurses_v1', JSON.stringify(res.nurses));
            localStorage.setItem('hemo_permanent_nurses_v1', JSON.stringify(res.nurses));
          }
          if (res.machines && res.machines.length > 0) {
            setMachines(res.machines);
            localStorage.setItem('hemo_machines_v1', JSON.stringify(res.machines));
          }
          if (res.bays && res.bays.length > 0) {
            setBays(res.bays);
            localStorage.setItem('hemo_custom_bays_v1', JSON.stringify(res.bays));
          }
          if (res.specialDutyOptions && res.specialDutyOptions.length > 0) {
            setSpecialDutyOptions(res.specialDutyOptions);
            localStorage.setItem('hemo_special_duty_options_v1', JSON.stringify(res.specialDutyOptions));
          }
          if (res.doctors && res.doctors.length > 0) {
            setDoctors(res.doctors);
            localStorage.setItem('hemo_doctors_v1', JSON.stringify(res.doctors));
          }
          if (res.doctorDuties && Object.keys(res.doctorDuties).length > 0) {
            setDoctorDuties((prev) => {
              const next = { ...prev, ...res.doctorDuties };
              localStorage.setItem('hemo_doctor_duties_v1', JSON.stringify(next));
              return next;
            });
          }
          if (res.assignments && res.assignments.length > 0) {
            const pulledDates = new Set(res.assignments.map((a) => a.date));
            setAssignments((prev) => {
              const next = [
                ...prev.filter((a) => !pulledDates.has(a.date)),
                ...res.assignments,
              ];
              localStorage.setItem('hemo_assignments_v1', JSON.stringify(next));
              return next;
            });
          }
          ...
          localStorage.setItem('hemo_last_sheets_sync_time', String(now));
          setSheetsSyncStatus('saved');
          setSheetsSyncMessage('Semua data mutakhir dari Google Sheets');
        } else {
          setSheetsSyncStatus('idle');
          setSheetsSyncMessage('Tersinkron dengan Google Sheets');
        }
      } catch {
        setSheetsSyncStatus('idle');
        setSheetsSyncMessage('Tersinkron dengan Google Sheets');
    ...
    const timer = setTimeout(performAutoFetch, 1200);
    return () => clearTimeout(timer);
  }, [settings.googleSheetWebhookUrl, selectedYear, selectedMonth]);

  // Listener status koneksi online/offline
  useEffect(() => {
    const handleOnline = () => {
      if (sheetsSyncStatus === 'offline' || sheetsSyncStatus === 'error') {
        setSheetsSyncStatus('idle');
        setSheetsSyncMessage('Koneksi internet pulih');
  ...
  const sanitizeSpecialDuty = (dutyStr?: string | null): string | null => {
    if (!dutyStr) return null;
    const parsed = parseSpecialDuties(dutyStr);
    const cleaned = parsed
      .filter((c) => !PURGED_SPECIAL_DUTY_CODES.has(c.toUpperCase()))
      .map((c) => (c === 'FARMASI & LOGISTIK' ? 'FARMASI LOGISTIK' : c));
    return formatSpecialDuties(cleaned);
  };
  ...
  const sanitizeAssignmentForFirestore = (item: ShiftAssignment): ShiftAssignment => {
    return {
      ...
      notes: item.notes || '',
      specialDuty: sanitizeSpecialDuty(item.specialDuty),
    };
  };
  ...
  const syncNurseToCloud = async (nurse: Nurse) => {
    try {
      const sanitizedNurse: Nurse = {
        ...
        nip: nurse.nip || '',
        phone: nurse.phone || '',
        specialDuty: sanitizeSpecialDuty(nurse.specialDuty),
        ...
        (snapshot) => {
          ...
          if (!snapshot.empty) {
            ...
            const resetMonths = new Set<string>(
              JSON.parse(localStorage.getItem('hemo_reset_months_v1') || '[]')
            );

            const validAssignments = cloudAssignments.filter(
              (ca) =>
                !LEGACY_SAMPLE_NURSE_NAMES.has(ca.nurseName) &&
                !resetMonths.has(ca.date?.substring(0, 7))
            );
            ...
            setAssignments((prev) => {
              ...
              // excluding any months that were explicitly reset by Karu/Admin.
              const cloudDateNurseKeys = new Set(
                mergedCloudAssignments.map((a) => `${a.date}_${Number(a.nurseId)}`)
              );
              const allLocalCandidates = [...prev, ...localSavedAssignments].filter(
                (a) => a && a.date && !resetMonths.has(a.date.substring(0, 7))
              );
      ...
      unsubSettings = onSnapshot(
        ...
        (snapshot) => {
          if (snapshot.exists()) {
            ...
            if (!data.googleSheetWebhookUrl && INITIAL_SETTINGS.googleSheetWebhookUrl) {
              data.googleSheetWebhookUrl = INITIAL_SETTINGS.googleSheetWebhookUrl;
              updatePayload.googleSheetWebhookUrl = INITIAL_SETTINGS.googleSheetWebhookUrl;
              needsUpdate = true;
            }
            if (!data.googleSpreadsheetIdOrUrl && INITIAL_SETTINGS.googleSpreadsheetIdOrUrl) {
              data.googleSpreadsheetIdOrUrl = INITIAL_SETTINGS.googleSpreadsheetIdOrUrl;
              updatePayload.googleSpreadsheetIdOrUrl = INITIAL_SETTINGS.googleSpreadsheetIdOrUrl;
              needsUpdate = true;
            }
            if (needsUpdate) {
              setDoc(doc(db, 'settings', 'config'), updatePayload, { merge: true }).catch(() => {});
            }
            setSettings(data);
          } else {
        ...
        (snapshot) => {
          if (snapshot.exists()) {
            ...
            if (data.options && Array.isArray(data.options) && data.options.length > 0) {
              ...
              // If Firestore still held old purged duties or unnormalized code, update Firestore immediately
              if (cleaned.length !== data.options.length) {
                setDoc(doc(db, 'settings', 'special_duties'), { options: cleaned }).catch(() => {});
              }
              const finalOptions = cleaned.length > 0 ? cleaned : DEFAULT_SPECIAL_DUTY_OPTIONS;
              setSpecialDutyOptions(finalOptions);
              localStorage.setItem('hemo_special_duty_options_v1', JSON.stringify(finalOptions));
            }
          } else {
            setDoc(doc(db, 'settings', 'special_duties'), { options: DEFAULT_SPECIAL_DUTY_OPTIONS }).catch(() => {});
          }
        },
        (err) => {
  ...
  const generateMonthlySchedule = async (monthStr?: string) => {
    ...
    setIsGenerating(true);
    try {
      // 1. Remove targetMonth from reset list so new schedule can sync seamlessly
      try {
        const resetMonths = new Set<string>(
          JSON.parse(localStorage.getItem('hemo_reset_months_v1') || '[]')
        );
        resetMonths.delete(targetMonth);
        localStorage.setItem('hemo_reset_months_v1', JSON.stringify(Array.from(resetMonths)));
      } catch {}

      // 2. Generate balanced schedule using FairSchedulerEngine
      const newMonthSchedule = FairSchedulerEngine.generateMonthlySchedule(
        y,
        m,
        nurses,
      ...
      // 3. Clean up obsolete docs for this month in Firestore to prevent stale ID conflicts
      try {
      ...
      // 4. Update local state and upload new standardized assignments
      setAssignments((prev) => {
        const existingMonthMap = new Map<string, ShiftAssignment>();
        prev.filter((a) => a.date.startsWith(prefix)).forEach((a) => {
          existingMonthMap.set(`${a.date}_${a.nurseId}`, a);
        ...
        const otherMonths = prev.filter((a) => !a.date.startsWith(prefix));
        const updated = [...otherMonths, ...mergedMonthSchedule];
        localStorage.setItem('hemo_assignments_v1', JSON.stringify(updated));
        syncBatchAssignmentsToCloud(mergedMonthSchedule);
        return updated;
      });
      ...
      showToast(`Gagal menyusun jadwal: ${err instanceof Error ? err.message : String(err)}`, 'error');
    } finally {
      setIsGenerating(false);
    }
  };

  const resetMonthlySchedule = async (monthStr?: string): Promise<boolean> => {
    if (!checkKaruPermission('mereset jadwal bulanan')) return false;
    const targetMonth = monthStr || currentMonth;
    const parts = targetMonth.split('-');
    const y = parseInt(parts[0], 10);
    const m = parseInt(parts[1], 10);
    ...
    setIsGenerating(true);
    try {
      // 1. Mark month in reset list so snapshot listener does not resurrect old assignments
      try {
        const resetMonths = new Set<string>(
          JSON.parse(localStorage.getItem('hemo_reset_months_v1') || '[]')
        );
      ...
      // 2. Delete assignments for this month from Firestore in batches
      try {
        const snap = await getDocs(collection(db, 'assignments'));
        const docsToDelete = snap.docs.filter((d) => {
          const data = d.data();
        ...
        console.warn('Could not batch delete from cloud:', cloudErr);
      }

      // 3. Clear local state and localStorage
      setAssignments((prev) => {
        const remaining = prev.filter((a) => !a.date.startsWith(prefix));
        localStorage.setItem('hemo_assignments_v1', JSON.stringify(remaining));
        return remaining;
      });

      triggerAutoSaveToGoogleSheets(true);
      showToast(
        `Jadwal bulan ${m}/${y} berhasil direset! Seluruh staf dan perawat tetap utuh tersimpan.`,
        'success'
      );
      return true;
    } catch (err) {
      showToast(`Gagal mereset jadwal: ${err instanceof Error ? err.message : String(err)}`, 'error');
      return false;
    } finally {
      setIsGenerating(false);
    }
  ...
  const setShift = (date: string, nurseId: number, shiftType: ShiftType) => {
    ...
    triggerAutoSaveToGoogleSheets(false);
  };
      ...
      const updatedMachines = machines.map((m) => {
        if (machineIds.includes(m.id)) {
          const updated: Machine = {
            ...
            statusPagi: status || m.statusPagi,
            statusSiang: status || m.statusSiang,
          };
  ...
  const updateBay = (
    ...
    applyStatusToAll: boolean = true,
    targetShift: 'ALL' | 'PAGI' | 'SIANG' = 'ALL'
  ) => {
    ...
    const affectedMachines: Machine[] = [];
    const shouldUpdateStatus = applyStatusToAll && Boolean(newStatus);
    ...
    const updatedMachines = machines.map((m) => {
      if (m.bay?.trim().toLowerCase() === trimmedOld.toLowerCase()) {
        const updated: Machine = {
          ...
          status: shouldUpdateStatus && newStatus ? (targetShift === 'ALL' ? newStatus : m.status) : m.status,
          statusPagi:
            shouldUpdateStatus && newStatus
              ? targetShift === 'ALL' || targetShift === 'PAGI'
                ? newStatus
                : m.statusPagi
              : m.statusPagi,
          statusSiang:
            shouldUpdateStatus && newStatus
              ? targetShift === 'ALL' || targetShift === 'SIANG'
                ? newStatus
                : m.statusSiang
              : m.statusSiang,
        };
    ...
    const statusMsg = shouldUpdateStatus && newStatus ? `, status diatur ke "${newStatus}"` : '';
    showToast(
      `Pengaturan Bay "${trimmedNewName}" berhasil disimpan (${affectedMachines.length} mesin disinkronkan${statusMsg}).`,
      'success'
    );
  };

  const setBayMachinesStatus = (
    bayName: string,
    status: MachineStatus,
    targetShift: 'ALL' | 'PAGI' | 'SIANG' = 'ALL'
  ) => {
    if (!checkKaruPermission('mengubah status operasional Bay')) return;
    const trimmedBay = bayName.trim().toLowerCase();
    const affectedMachines: Machine[] = [];

    const updatedMachines = machines.map((m) => {
      if (m.bay?.trim().toLowerCase() === trimmedBay) {
        const updated: Machine = {
          ...m,
          status: targetShift === 'ALL' ? status : m.status,
          statusPagi: targetShift === 'ALL' || targetShift === 'PAGI' ? status : m.statusPagi,
          statusSiang: targetShift === 'ALL' || targetShift === 'SIANG' ? status : m.statusSiang,
        };
        affectedMachines.push(updated);
        return updated;
      }
    ...
    if (affectedMachines.length === 0) {
      showToast(`Tidak ada mesin yang terdaftar di ${bayName}.`, 'info');
      return;
    }
    ...
    const shiftLabel = targetShift === 'ALL' ? 'Semua Sif' : targetShift === 'PAGI' ? 'Sif Pagi' : 'Sif Siang';
    showToast(
      `Status operasional seluruh ${affectedMachines.length} mesin di "${bayName}" berhasil diubah menjadi ${status} (${shiftLabel}).`,
      'success'
    );
  };
  ...
  const syncWithGoogleSheets = async (customUrl?: string): Promise<boolean> => {
    ...
    try {
      ...
      const msg = `Gagal sinkron: ${err instanceof Error ? err.message : String(err)}`;
      showToast(msg, 'error');
      return false;
    } finally {
      setIsSyncing(false);
  ...
  const syncAllToGoogleSheets = async (customUrl?: string): Promise<{ success: boolean; message: string }> => {
    ...
    try {
      ...
      if (result.isSuccess) {
        ...
        setSheetsSyncStatus('error');
        setSheetsSyncMessage(result.message);
      }
      showToast(result.message, result.isSuccess ? 'success' : 'error');
  ...
  const fetchDataFromGoogleSheets = async (
    ...
    try {
      ...
      // 4. Update special duty options (Master Tugas Khusus)
      if (result.specialDutyOptions && result.specialDutyOptions.length > 0) {
        setSpecialDutyOptions(result.specialDutyOptions);
        localStorage.setItem('hemo_special_duty_options_v1', JSON.stringify(result.specialDutyOptions));
      }
      ...
      // 8. Background sync to Firestore if possible (silently ignore quota exhaustion)
      try {
        if (db) {
          ...
          await setDoc(
            syncDoc,
            {
              ...
              specialDutyOptions: result.specialDutyOptions && result.specialDutyOptions.length > 0 ? result.specialDutyOptions : specialDutyOptions,
              updatedAt: Date.now(),
  ...
  const syncAllDataToCloud = async (): Promise<boolean> => {
    ...
    try {
      ...
      await setDoc(doc(db, 'settings', 'special_duties'), { options: specialDutyOptions }, { merge: true });
      setIsCloudConnected(true);
      showToast(
  ...
  const fetchDataFromCloud = async (): Promise<{ success: boolean; nurses: number; machines: number; assignments: number }> => {
    setIsSyncing(true);
    try {
      let nursesSnap, machinesSnap, assignmentsSnap, settingsSnap, baysSnap, specialDutiesSnap;
      try {
        // Force fresh fetch directly from server
        [nursesSnap, machinesSnap, assignmentsSnap, settingsSnap, baysSnap, specialDutiesSnap] = await Promise.all([
          ...
          getDocFromServer(doc(db, 'settings', 'special_duties')),
        ]);
      } catch (directErr) {
        ...
        [nursesSnap, machinesSnap, assignmentsSnap, settingsSnap, baysSnap, specialDutiesSnap] = await Promise.all([
          ...
          getDoc(doc(db, 'settings', 'special_duties')),
        ]);
      }
      ...
      if (specialDutiesSnap && specialDutiesSnap.exists()) {
        const sdData = specialDutiesSnap.data() as { options?: SpecialDutyOption[] };
        if (sdData && Array.isArray(sdData.options) && sdData.options.length > 0) {
          setSpecialDutyOptions(sdData.options);
          localStorage.setItem('hemo_special_duty_options_v1', JSON.stringify(sdData.options));
        }
      }
  ...
  const updateSpecialDutyOption = async (id: string, updatedFields: Partial<SpecialDutyOption>) => {
    const original = specialDutyOptions.find((o) => o.id === id);
    const updated = specialDutyOptions.map((o) => (o.id === id ? { ...o, ...updatedFields } : o));
    setSpecialDutyOptions(updated);
    localStorage.setItem('hemo_special_duty_options_v1', JSON.stringify(updated));

    // If code has changed, cascade rename into existing nurses and assignments
    if (original && updatedFields.code && original.code.trim().toUpperCase() !== updatedFields.code.trim().toUpperCase()) {
      const oldCode = original.code.trim();
      const newCode = updatedFields.code.trim().toUpperCase();

      // Cascade to nurses
      const updatedNurses = nurses.map((n) => {
        if (n.specialDuty) {
          const list = parseSpecialDuties(n.specialDuty);
          if (list.includes(oldCode)) {
            const newList = list.map((d) => (d === oldCode ? newCode : d));
            return { ...n, specialDuty: formatSpecialDuties(newList) };
          }
        }
        return n;
      });
      setNurses(updatedNurses);
      localStorage.setItem('hemo_nurses_v1', JSON.stringify(updatedNurses));
      syncAllNursesToCloud(updatedNurses);

      // Cascade to assignments
      const updatedAssignments = assignments.map((a) => {
        if (a.specialDuty) {
          const list = parseSpecialDuties(a.specialDuty);
          if (list.includes(oldCode)) {
            const newList = list.map((d) => (d === oldCode ? newCode : d));
            return { ...a, specialDuty: formatSpecialDuties(newList) };
          }
        }
        return a;
      });
      setAssignments(updatedAssignments);
      localStorage.setItem('hemo_assignments_v1', JSON.stringify(updatedAssignments));
      const affectedAssignments = updatedAssignments.filter(
        (a) => a.specialDuty && parseSpecialDuties(a.specialDuty).includes(newCode)
      );
      if (affectedAssignments.length > 0) {
        syncBatchAssignmentsToCloud(affectedAssignments);
      }
    }
